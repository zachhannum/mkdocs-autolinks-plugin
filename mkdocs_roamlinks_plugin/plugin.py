...
class AutoLinkReplacer:
    ...
    def __call__(self, match):
        ...
        # Absolute URL of the linker  # 看起来是page的dirtory文件夹
        ...
        for root, dirs, files in os.walk(self.base_docs_url):
            for name in files:
                ...
                if name == filename:
                    # Absolute path to the file we want to link to  # 直接abs_link_url 等于root 就可以？还是需要转换路径？
            ...
            for root, dirs, files in os.walk(self.base_docs_url):
                for name in files:
                    ...
                    # if name == filename:
                        ...
                        # Constructing relative path from the linker to the link
                        if title:
                            rel_link_url = os.path.join(os.path.relpath(abs_link_url, abs_linker_url), name.replace('.md', ''), '#' + format_title)
                        else:
...
class RoamLinksPlugin(BasePlugin):
    ...
    def on_page_markdown(self, markdown, page, config, site_navigation=None, **kwargs):
        ...
        markdown = re.sub(ROAMLINK_RE, RoamLinkReplacer(base_docs_url, page_url), markdown)
